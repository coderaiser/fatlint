...
test('flatlint: traverser: path.getNextSibling(): not-found', async (t) => {
    const disk = await createDisk();
    const filesystem = createFilesystem(disk);
    const source = `const a = 'hello';`;
    const ast = parse(source);
...
test('flatlint: traverser: path.getPrevSibling()', async (t) => {
    const disk = await createDisk();
    const filesystem = createFilesystem(disk);
    const source = `const a = 'hello'; const b = 'world';`;
    const ast = parse(source);
...
test('flatlint: traverser: path.find()', async (t) => {
    const disk = await createDisk();
    const filesystem = createFilesystem(disk);
...
test('flatlint: traverser: path.find(): not found', async (t) => {
    const disk = await createDisk();
    const filesystem = createFilesystem(disk);
    const source = `function x() {const a = 'hello';}`;
    const ast = parse(source);
